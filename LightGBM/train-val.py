...
def main():
    ...
    # joblib.dump(scaler, './AICUP/models/LightGBM/scaler.pkl')
    ...
    def model_binary(X_train, y_train, X_test, y_test, model_name):
        ...
        # 如果檔案存在，讀取舊資料
        if os.path.exists('./AICUP/LightGBM/best_results.json'):
            with open('./AICUP/LightGBM/best_results.json', 'r') as f:
                data = json.load(f)
        else:
            data = []
        ...
        # 儲存更新後的檔案
        with open('./AICUP/LightGBM/best_results.json', 'w') as f:
        ...
        # ax.set_title(f"Feature importances using permutation on full model {model_name}")l
        # ax.set_ylabel("Mean accuracy decrease")
        # fig.tight_layout()
        # plt.savefig(f'./AICUP/LightGBM/{model_name}_binary_importance.png')
        # plt.show()
        

        # Save model
        # joblib.dump(clf, f'./AICUP/models/{model_name}_binary.pkl')
        
        predicted = search.predict_proba(X_test)
        # 取出正類（index 0）的概率
    ...
    def model_multiary(X_train, y_train, X_test, y_test, model_name):
        ...
        clf = LGBMClassifier(boosting_type="gbdt", objective='multiclass', num_class=len(np.unique(y_train)))       
        ...
        # fig, ax = plt.subplots()
        # forest_importances.plot.bar(yerr=result.importances_std, ax=ax)
        # ax.set_title(f"Feature importances using permutation on full model {model_name}")
        # ax.set_ylabel("Mean accuracy decrease")
        # fig.tight_layout()
        # plt.show()
        # plt.savefig(f'./AICUP/LightGBM/{model_name}_multiary_importance.png')
        
        # Save model
        # joblib.dump(clf, f'./AICUP/models/{model_name}_multiary.pkl')
        ...
        print(f'{model_name} Multiary AUC: {auc_score:.3f}')
        

    # 評分：針對各目標進行模型訓練與評分
    y_train_le_gender = le.fit_transform(y_train['gender'])
    #joblib.dump(le, './AICUP/models/le gender.pkl')
    y_test_le_gender = le.transform(y_test['gender'])
    model_binary(X_train_scaled, y_train_le_gender, X_test_scaled, y_test_le_gender, "gender")
    # model_binary(X_train_scaled, y_train_le_gender, None, None, "gender")
    
    y_train_le_hold = le.fit_transform(y_train['hold racket handed'])
    #joblib.dump(le, './AICUP/models/le hold racket handed.pkl')
    y_test_le_hold = le.transform(y_test['hold racket handed'])
    model_binary(X_train_scaled, y_train_le_hold, X_test_scaled, y_test_le_hold, 'hold_racket_handed')
    # model_binary(X_train_scaled, y_train_le_hold, None, None, 'hold racket handed')
    
    y_train_le_years = le.fit_transform(y_train['play years'])
    #joblib.dump(le, './AICUP/models/le play years.pkl')
    y_test_le_years = le.transform(y_test['play years'])
    model_multiary(X_train_scaled, y_train_le_years, X_test_scaled, y_test_le_years, 'play_years')
    # model_multiary(X_train_scaled, y_train_le_years, None, None, 'play years')

    y_train_le_level = le.fit_transform(y_train['level'])
    #joblib.dump(le, './AICUP/models/le level.pkl')
    y_test_le_level = le.transform(y_test['level'])
    model_multiary(X_train_scaled, y_train_le_level, X_test_scaled, y_test_le_level, 'level')
    # model_multiary(X_train_scaled, y_train_le_level, None, None, 'level')

    # gbdt::: gender: 0.889, hoold racket: 1, play years: 0.637, level: 0.815
    # dart::: gender: 0.901, hoold racket: 0.999, play years: 0.662, level: 0.832
    # rf::::: gender: 0.599, hoold racket: 0.992, play years: 0.630, level: 0.7
