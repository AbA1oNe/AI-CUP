...
def main():
    ...
    def model_binary(X_train, y_train, X_test, y_test, model_name):
        ...
        if model_name == "gender":
            clf = LGBMClassifier(boosting_type="gbdt", random_state=42, colsample_bytree=0.5627350266825741,
            learning_rate= 0.14880382550253787,
            max_depth= -1,
            min_child_samples= 59,
            n_estimators= 212,
            num_leaves= 125,
            subsample= 0.9518071723647965)
        elif model_name == "hold racket handed":
            clf = LGBMClassifier(boosting_type="gbdt", random_state=42, colsample_bytree=0.462438463084525,
            learning_rate= 0.17383004516837042,
            max_depth= 9,
            min_child_samples= 10,
            n_estimators= 505,
            num_leaves= 112,
            subsample= 0.7623209877330608)
        clf.fit(X_train, y_train)
        

        # Save model
        joblib.dump(clf, f'./AICUP/models/LightGBM/{model_name}_binary.pkl')
        

    # 定義多類別分類評分函數 (例如 play years、level)
    def model_multiary(X_train, y_train, X_test, y_test, model_name):
        if model_name == "play years":
            clf = LGBMClassifier(boosting_type="gbdt", random_state=42, colsample_bytree=0.4903532552550374,
            learning_rate= 0.0990431413688412,
            max_depth= 14,
            min_child_samples= 49,
            n_estimators= 731,
            num_leaves= 104,
            subsample= 0.7835324695684127)
        elif model_name == "level":
            clf = LGBMClassifier(boosting_type="gbdt", random_state=42, colsample_bytree=0.5007925207380406,
            learning_rate= 0.12981873122563972,
            max_depth= 12,
            min_child_samples= 33,
            n_estimators= 761,
            num_leaves= 123,
            subsample= 0.8287596300929925)
        clf.fit(X_train, y_train)
        
        # Save model
        joblib.dump(clf, f'./AICUP/models/LightGBM/{model_name}_multiary.pkl')
        

    # 評分：針對各目標進行模型訓練與評分
    y_train_le_gender = le.fit_transform(y_train['gender'])
    joblib.dump(le, './AICUP/models/LightGBM/le gender.pkl')
    ...
    joblib.dump(le, './AICUP/models/LightGBM/le hold racket handed.pkl')
    ...
    joblib.dump(le, './AICUP/models/LightGBM/le play years.pkl')
    ...
    joblib.dump(le, './AICUP/models/LightGBM/le level.pkl')
