...
def main():
    ...
    def model_binary(X_train, y_train, X_test, y_test):
        ...
        joblib.dump(clf, 'hold racket handed_binary.pkl')
    ...
    def model_multiary(X_train, y_train, X_test, y_test):
        clf = LogisticRegression(penalty='l2', solver='liblinear', random_state=42)
        clf.fit(X_train, y_train)
        joblib.dump(clf, 'level_multiary.pkl')
        predicted = clf.predict_proba(X_test)
        num_groups = len(predicted) // group_size
        y_pred = []
    ...
    joblib.dump(le, 'le hold racket handed.pkl')
    ...
    joblib.dump(le, 'le play years.pkl')
