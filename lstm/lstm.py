...
for t in targets:
    ...
    if len(le.classes_) == 2:
        y_outputs[t] = np.array(y).reshape(-1, 1)  # 二元分類
    else:
...
for t in targets:
    ...
    if y_outputs[t].shape[1] == 1:
        output = Dense(1, activation='sigmoid', name=name)(lstm_out)
        losses[name] = 'binary_crossentropy'
    else:
        output = Dense(y_outputs[t].shape[1], activation='softmax', name=name)(lstm_out)
...
# 6. 訓練
model.fit(x_data, [y_outputs[t] for t in targets], epochs=20, batch_size=32, validation_split=0.1)
